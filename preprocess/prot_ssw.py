...
def read_prot_dict(prot_dict_path):
    with open(prot_dict_path, 'r') as load_f:
        load_dict = json.load(load_f)
    return load_dict


prot_dict = read_prot_dict(prot_dict_path)
num = len(prot_dict)
    ...
    for sQId, sQSeq, sQQual in read(args.query):
        ...
        for sRId, sRSeq, _ in read(args.target):
            ...
            if not args.bSam:
                ...
                target_idx = int(sRId)
                query_idx = int(sQId)
                distance_matrix[target_idx][query_idx] = resPrint[0]
                if resPrint[1] > 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--bProtein', action='store_true',
    ...
    parser.add_argument('-r', '--bBest', action='store_true',
    ...
    parser.add_argument('-header', '--bHeader', action='store_true',
                        help='If -s is used, include header in SAM output.')
    parser.add_argument('-t', '--target', default='/home/s2136015/Code/DeepTarget_final/data/protein',
                        help='target file')
    parser.add_argument('-q', '--query', default='/home/s2136015/Code/DeepTarget_final/data/protein', help='query file')
    if len(sys.argv) == 1:
        parser.print_help()
        sys.exit()
    ...
    np.save("prot_cluster/prot_ssw.npy", distance_matrix)
