...
class BackwardRunner(ForwardRunner):
    ...
    def save_state(self, save_directory: typing.Union[str, Path], epoch_id: int):
        # NOTE: ADD SAVE_FEEQ
        save_directory = Path(f"{save_directory}/{epoch_id}")
...
def run_valid_epoch(epoch_id: int,
                    ...
                    viz: typing.Optional[visualization.TAPEVisualizer] = None,
                    is_master: bool = True, 
                    task: str = None) -> typing.Tuple[float, typing.Dict[str, float]]:
 
    is_regression = True if task in ["fluorescence", "stability"] else False
    ...
    for batch in tqdm(valid_loader, desc='Running Eval', total=num_batches,
        ...
        # loss, metrics, outputs= runner.forward(batch)  # type: ignore
    ...
    # only for regression task, spearmanr metric is absent from returns of runner.forward()
    if is_regression:
        metrics_arg = ["spearmanr"]
        metric_functions = [registry.get_metric(name) for name in metrics_arg]
        ...
        metrics_to_save = {name: metric(target, prediction)
                        for name, metric in zip(metrics_arg, metric_functions)}

        # print("logger_INFO", ''.join(f'{name}: {val}' for name, val in metrics_to_save.items()))
    ...
    if is_regression:
        metrics['spearmanr'] = metrics_to_save["spearmanr"]
    if viz is not None:
...
def run_train(model_type: str,
              ...
              time_or_name: str = 'time',
              force_save: str = 'FALSE',
    ...
    exp_dir = utils.get_expname(exp_name, task, model_type, time_or_name)
    ...
    optimizer = utils.setup_optimizer(model, learning_rate, weight_decay)
    ...
    # Do force save: get an untrained  model quickly
    if force_save != 'FALSE':
        Path(force_save).mkdir(parents=True, exist_ok=True)
        runner.save_state(force_save, 0)
        print("Saved model at {}".format(force_save))
        exit(0)
    ...
    with utils.wrap_cuda_oom_error(local_rank, batch_size, n_gpu, gradient_accumulation_steps):
        for epoch_id in range(start_epoch, num_train_epochs):
            ...
            if eval_freq > 0 and (epoch_id + 1) % eval_freq == 0:
                val_loss, _ = run_valid_epoch(epoch_id, valid_loader, runner, viz, is_master, task)
            ...
            if patience > 0 and num_evals_no_improvement >= patience:
                ...
                if local_rank != -1:
                    ...
                    break
    val_loss, _ = run_valid_epoch(1, valid_loader, runner, viz, is_master, task)
...
def run_eval(model_type: str,
    ...
    if 'L5' in metrics:
        _, metrics_L5 = run_valid_epoch(1, valid_loader, runner, task='contact_prediction')
        metrics_to_save['precision_at_l5'] = metrics_L5['precision_at_l5']
    ...
    with (pretrained_dir / 'results.pkl').open('wb') as f:
        pkl.dump((metrics_to_save, save_outputs), f)
    # run_valid_epoch(1, valid_loader, runner, viz=None, is_master=is_master)
    return metrics_to_save
