...
class ProteinBertSelfAttention(nn.Module):
    """
        DOC:    self attn is used in bert-attn, followed by a dense layer (mapping h to h)
                Consists of q, k, v, and dropout.
    """
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        attention_mask = (attention_mask < -9999)
...
class ProteinBertAttention(nn.Module):
    """
        DOC:        This is the second block in BertLayer, there are 4 in all (INPUT_LN, ATTN, POST_ATTN_LN, MLP)
                    Consist of a self-attn and a dense layer (dense is used to affine output of self-attn)
    """
...
class ProteinBertFFN(nn.Module):
    """
        DOC:        This is the fourth block in BertLayer, there are 4 in all (INPUT_LN, ATTN, POST_ATTN_LN, MLP)
                    Consist of 2 dense layers: hidden_size -> 4 * hidden_size, 4 * hidden_size -> hidden_size
                    Activation function between 2 dense layers: GeLU
    """

    def __init__(self, config):
        super().__init__()
        self.dense_h_4h = nn.Linear(config.hidden_size, config.intermediate_size)
        self.dense_4h_h = nn.Linear(config.intermediate_size, config.hidden_size)
    ...
    def forward(self, hidden_states):
        intermediate = self.dense_h_4h(hidden_states)
        activated_intermediate = self.intermediate_act_fn(intermediate)
        hidden_states = self.dense_4h_h(activated_intermediate)
        return hidden_states


# Protein Bert Layer Structure
# 1. Input LN
# 2. Attn (SelfAttn + Dense)
# 3. Post Attn LN
# 4. Bert Output (MLP: h -> 4h -> h)
class ProteinBertLayer(nn.Module):
    """
        DOC:        Consist 4 blocks in all (INPUT_LN, ATTN, POST_ATTN_LN, MLP)
                    There are 2 residual connections:
                            (res1 start) INPUT_LN, ATTN (res1 end)
                            (res2 start) POST_ATTN_LN, MLP (res2 end)
    """

    def __init__(self, config):
        super().__init__()
        self.input_ln = LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.attention = ProteinBertAttention(config)
        self.post_attn_ln = LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.mlp = ProteinBertFFN(config)

    def forward(self, hidden_states, attention_mask):
        # Input LN
        ln_output = self.input_ln(hidden_states)
        # Attn
        attn_outs = self.attention(ln_output, attention_mask)
        attn_out = attn_outs[0]

        # Residual connection1
        res1 = attn_out + hidden_states

        # Post Attn LN
        post_attn_ln_output = self.post_attn_ln(res1)
        # MLP
        mlp_output = self.mlp(post_attn_ln_output)

        # Residual connection2
        res2 = mlp_output + res1
        # Output: concat results and trailing attn
        outputs = (res2,) + attn_outs[1:]
        return outputs


class ProteinBertEncoder(nn.Module):
    """
        DOC:        Beside the normal `num_layers` of BertLayer stacked layer by layer,
                        a final layer norm is also added to the tail of BertEncoder (numerical stability)
    """
    ...
    def __init__(self, config):
        ...
        self.final_ln = LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
    ...
    def run_function(self, start, chunk_size):
        def custom_forward(hidden_states, attention_mask):
            ...
            for index, layer in enumerate(self.layer[chunk_slice]):
...
class ProteinBertPooler(nn.Module):
    """
        DOC:        Take out the hidden state corresponding to the first token `[CLS]` as pooling results.
    ...
    def forward(self, hidden_states):
        first_token_pool = hidden_states[:, 0]
...
class ProteinBertModel(ProteinBertAbstractModel):
    ...
    def __init__(self, config):
        ...
        # will not be transferred from Megatron. Random inited
        self.bert_tail_dense = nn.Linear(config.hidden_size, config.hidden_size)
