...
class SimpleMLP(nn.Module):
    ...
    def __init__(self,
        ...
        if simple_mlp:
            ...
            self.main = nn.Sequential(
                weight_norm(nn.Linear(in_dim, hid_dim * 4), dim=None),
                nn.ReLU(),
                nn.Dropout(dropout, inplace=True),
                weight_norm(nn.Linear(hid_dim * 4, hid_dim * 2), dim=None),
                nn.ReLU(),
                nn.Dropout(dropout, inplace=True),
...
class ValuePredictionHead(nn.Module):
    def __init__(self, hidden_size: int, dropout: float = 0., fat_mlp: bool = True):
        super().__init__()
        if fat_mlp:
            self.value_prediction = SimpleMLP(hidden_size, 1024, 1, dropout)
        else:
    ...
    def forward(self, pooled_output, targets=None):
        ...
        if targets is not None:
            ...
            metrics = {'spearnmanr':
                           spearmanr(value_pred.detach().to('cpu').view(-1), targets.detach().to('cpu').view(-1))}
        return outputs  # (loss), value_prediction


class SequenceClassificationHead(nn.Module):
    def __init__(self, hidden_size: int, num_labels: int, fat_mlp: bool = True):
        super().__init__()
        if fat_mlp:
            self.classify = SimpleMLP(hidden_size, 1024, num_labels)
        else:
...
class PairwiseContactPredictionHead(nn.Module):
    def __init__(self, hidden_size: int, ignore_index=-100):
        super().__init__()
        self.predict = nn.Sequential(nn.Dropout(), nn.Conv2d(2 * hidden_size, 64, 1))
        for num in range(30):
            self.predict.add_module("ResidualBlock" + str(num), BasicBlock(64, 64))

        self.linear = nn.Sequential(nn.Linear(64, 2))
        self._ignore_index = ignore_index
    ...
    def forward(self, inputs, sequence_lengths, targets=None):
        ...
        prediction_conv = self.predict(pairwise_features)
        prediction_conv = prediction_conv.permute(0, 2, 3, 1)
        prediction = self.linear(prediction_conv)
